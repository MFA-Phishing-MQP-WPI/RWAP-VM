...
gnome-terminal -- bash -c "
    ...
    iptables -t nat -A PREROUTING -i $INTERFACE  -p tcp --dport 443 -j REDIRECT --to-port 8021;
    iptables -t nat -A PREROUTING -i $INTERFACE -p tcp --dport 80 -j REDIRECT --to-port 8021;
    mitmproxy -s intercept.py --ssl-insecure --mode transparent --listen-port 8021
    exec bash
"
